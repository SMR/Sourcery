...
extension Enum.Case {
    func diffAgainst(_ object: Any?) -> DiffableResult {
        ...
        if self.rawValue != rhs.rawValue { results.append("Incorrect rawValue, expected \(self.rawValue), received: \(rhs.rawValue)") }
        if self.associatedValues != rhs.associatedValues { results.append("Incorrect associatedValues, expected \(self.associatedValues), received: \(rhs.associatedValues)") }
